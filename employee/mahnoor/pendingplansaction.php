...
if (isset($_SESSION["login"])) {
    if ($_SESSION["login"] == true) {
        ...
        if ($action == 'generated') {
            ...
            $q1 = "insert into package (client_id , package_name,payment_id,package_date) values ((SELECT client_id from payment_approvals where approval_id = '" . $apid . "'),(SELECT purpose from payment_approvals where approval_id = '" . $apid . "'),(SELECT payment_id from payment_approvals where approval_id = '" . $apid . "'),DATE_ADD(now(), INTERVAL 2 DAY));";
            mysqli_query($conn, $q1);

            header("Location: ../../employee/mahnoor/pendingplans.php?");
        } else if ($action == 'edit') {
            ...
            header("Location: Location: ../../employee/mahnoor/pendingplans.php?submit=$submit");
        }
?>
